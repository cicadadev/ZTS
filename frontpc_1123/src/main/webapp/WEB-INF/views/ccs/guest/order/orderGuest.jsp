...
<jsp:include page="/WEB-INF/views/gsf/layout/common/navi.jsp" flush="false">
	...
	<jsp:param value="고객센터|비회원 주문조회" name="pageNavi"/>
</jsp:include>

<div class="inner">
<div class="layout_type1 csnon">
					...
					<c:forEach var="payment" varStatus="status" items="${paymentList}">
						<c:if test="${payment.majorPaymentYn == 'Y' && payment.paymentStateCd != 'PAYMENT_STATE_CD.CANCEL'}">
							<ul>
								...
								<!-- 영수증 -->								
								<li>
									<c:if test="${payment.paymentMethodCd == 'PAYMENT_METHOD_CD.VIRTUAL' || payment.paymentMethodCd == 'PAYMENT_METHOD_CD.TRANSFER'}">
										<c:if test="${!empty payment.cashReceiptApprovalNo && payment.paymentStateCd == 'PAYMENT_STATE_CD.PAYMENT'}">
											<a href="javascript:void(0);" class="btn_sStyle1" onclick="oms.printCashReceipt('${order.orderId}','${payment.pgApprovalNo }')">현금영수증</a>
										</c:if>
									</c:if>
									<c:if test="${payment.paymentMethodCd == 'PAYMENT_METHOD_CD.CARD' || payment.paymentMethodCd == 'PAYMENT_METHOD_CD.MOBILE'}">
										<a href="javascript:void(0);" class="btn_sStyle1" onclick="oms.printReceipt('${order.orderId}','${payment.pgApprovalNo }')">영수증</a>
									</c:if>
									<c:if test="${payment.paymentMethodCd == 'PAYMENT_METHOD_CD.KAKAO'}">
										<a href="javascript:void(0);" class="btn_sStyle1" onclick="oms.printReceiptKakao('${payment.pgApprovalNo }')">카카오전표</a>
									</c:if>
								</li>
							</ul>
						</c:if>
...
<script type="text/javascript" src="/resources/js/ccs/ccs.guest.js"></script>	