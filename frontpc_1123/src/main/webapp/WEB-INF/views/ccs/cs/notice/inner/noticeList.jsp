		...
		<div class="list_group">
								...
								<div class="tr_box">
									...
									<div class="col2">
										<div class="text_indent">
											<a href="javascript:custcenter.notice.detail('${list.noticeNo}', '${list.readCnt}');" class="name" >
												<i>
													<c:if test="${list.topNoticeYn == 'Y'}">
														<span class="icon_type1 iconPurple2 mo_only">공지</span>
													</c:if>
													<c:if test="${!empty list.brandName}">
														[${list.brandName}]
													</c:if>
													${list.title}
			...
			<div class="paginateType1">
				<page:paging formId="noticeSearchForm" currentPage="${search.currentPage}" pageSize="${search.pageSize}" 
						total="${totalCount}" url="/ccs/cs/notice/list/ajax" type="ajax" callback="custcenter.notice.listCallback"/>
			</div>
			<!-- ### //PC 페이징 ### -->
		</div>			
 	</c:when>
