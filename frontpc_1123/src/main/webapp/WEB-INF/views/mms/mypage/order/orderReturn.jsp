...
<jsp:include page="/WEB-INF/views/gsf/layout/common/navi.jsp" flush="false">
	<jsp:param value="마이쇼핑|MY주문관리|반품신청" name="pageNavi"/>
</jsp:include>
<div class="inner">
<div class="layout_type1">
<div class="column">
	<div class="myorder claimApply return">
		<h3 class="title_type1">반품신청</h3>
		<!-- 16.09.27 : 텍스트수정 -->
		<div class="borderBox">
			<dl class="order_date">
				<dt>주문일시</dt>
...
<input name="claimTypeCd" value="CLAIM_TYPE_CD.RETURN" type="hidden" style="display: none;">
<input name="claimStateCd" value="CLAIM_STATE_CD.REQ" type="hidden" style="display: none;">

<c:set var="returnDeliveryFee" value="0"/>
<c:forEach var="address" varStatus="idx1" items="${order.omsDeliveryaddresss}">
	<c:forEach var="delivery" varStatus="idx2" items="${address.omsDeliverys}">
		<c:set var="returnDeliveryFee" value="${delivery.deliveryFee}"/>
	</c:forEach>
</c:forEach>
	...
	<c:if test="${product2.orderProductTypeCd == 'ORDER_PRODUCT_TYPE_CD.ORDERPRESENT'}">
		<c:if test="${!fn:contains(promotionIds, product2.presentId) && !empty product2.presentId}">
			...
			<div class="promotion full"
				...
				style="display: none;">
				<strong><span class="icon_type1 iconPurple2">반품사은품</span>${product2.presentName}</strong>
				<ul>
			...
			<div class="box toggleCont">
				<div class="columnL">
					<dl id="refund_layer">
						...
						<dd><em class="minus">0</em><i>원</i></dd>
					</dl>
					<dl class="detail">
						<c:forEach var="payment" varStatus="status" items="${paymentList}">
							<c:if test="${payment.paymentMethodCd == 'PAYMENT_METHOD_CD.POINT' || payment.paymentMethodCd == 'PAYMENT_METHOD_CD.DEPOSIT'}">
