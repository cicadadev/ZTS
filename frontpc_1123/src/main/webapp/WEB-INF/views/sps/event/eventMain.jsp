				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach items="${eventList}" var="event" varStatus="status1">
							...
							<li>
								<a href="javascript:ccs.link.go('/sps/event/detail?eventId=${event.eventId}', CONST.NO_SSL);">
									<div class="img">									
										<c:if test="${event.eventTypeCd eq 'EVENT_TYPE_CD.COUPON'}">
											<span class="tag">
						...
						<c:forEach items="${expEventList}" var="expEvent" varStatus="status2">
							...
							<li>							
								<dl class="item_info">
									...
									<dd class="title">${expEvent.name}</dd>								
